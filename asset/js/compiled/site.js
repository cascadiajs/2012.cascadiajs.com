...
(function( $, window, document, undefined ) {
	...
	$.fn.miniTwitter = function( options ) {
		...
		function url() {
			...
			function createdAt (date) {
				...
				if (d>86400000) {
					...
					d = parseInt(d/3600000, 10)+'h';
				} else if (d>60000) {
					d = parseInt(d/60000, 10)+'m';
				} else if (d>1000) {
					d = parseInt(d/1000, 10)+'s';
				}
				return d;
			}
		...
		function fetchTweetsData ( object ) {
			...
			obj.avatar = '<div class="tweet" tweetid='+obj.tweetId+'><div class="mt_avatar"><a '+rel()+' '+target()+' class="mt_avatar" href="'+obj.userUrl+'"><img src="'+obj.image+'" alt="'+obj.realName+'\'s avatar" border="0"/></a></div>';
		...
		function display ( widget ) {
			...
			fetch().done(function( res ) {
				...
				if ($(widget).find(".tweet").length > 0) {
					var existing_id = $(widget).find('.tweet').first().attr('tweetid');
					var new_tweets = [];

					for(i=0; i<tweets.length; i++) {
						if (tweets[i].tweetId == existing_id) {
							break;
						}
					...
					for(i=0; i<new_tweets.length; i++) {
						...
						$(widget).prepend(tweet.avatar+tweet.header+tweet.textTweet+tweet.footer);
						$(widget).find('.tweet').last().remove();
					}
